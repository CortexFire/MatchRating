...
begin
  ...
  values (v_match_id, 1, v_actor, p_format, 'Initial submission', 'active')
...
begin
  ...
  if char_length(coalesce(btrim(p_reason), '')) not between 2 and 600 then
    raise exception using errcode = 'MRVAL', message = 'Invalid revision reason';
  end if;
...
begin
  ...
  if p_action = 'disputed'
    and char_length(coalesce(btrim(p_note), '')) not between 2 and 600 then
    raise exception using errcode = 'MRVAL', message = 'A dispute note between 2 and 600 characters is required';
  end if;
  ...
  insert into public.match_confirmations (revision_id, user_id, action, note)
  values (
    p_revision_id,
    auth.uid(),
    p_action,
    case when p_action = 'disputed' then btrim(p_note) else null end
  );
...
revoke all on function public.command_revise_match(uuid, uuid, uuid, text, public.match_format, uuid[], uuid[], jsonb) from public, anon, authenticated;
revoke all on function public.command_review_match(uuid, uuid, public.confirmation_action, text) from public, anon, authenticated;
...
grant execute on function public.command_revise_match(uuid, uuid, uuid, text, public.match_format, uuid[], uuid[], jsonb) to authenticated;
grant execute on function public.command_review_match(uuid, uuid, public.confirmation_action, text) to authenticated;
