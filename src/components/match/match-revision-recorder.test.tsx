...
const actionMocks = vi.hoisted(() => ({ correctMatch: vi.fn(), reviseMatch: vi.fn() }));
...
test("submits a correction through the atomic action without free-text metadata", async () => {
  actionMocks.correctMatch.mockResolvedValue({
    ok: true,
  ...
  await waitFor(() => expect(actionMocks.correctMatch).toHaveBeenCalledWith({
