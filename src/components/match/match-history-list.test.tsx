...
describe("MatchHistoryList", () => {
  ...
  test("retains stored format and confirmed-status search terms after hiding their labels", () => {
    render(<MatchHistoryList matches={[{ ...matches[0], status: "confirmed" }]} />);

    fireEvent.change(screen.getByPlaceholderText("Search matches"), { target: { value: "singles" } });
    expect(screen.getByText("Alice Tan vs Bea Rivera")).toBeTruthy();

    fireEvent.change(screen.getByPlaceholderText("Search matches"), { target: { value: "confirmed" } });
    expect(screen.getByText("Alice Tan vs Bea Rivera")).toBeTruthy();
    expect(screen.queryByText("Accepted")).toBeNull();
  });
